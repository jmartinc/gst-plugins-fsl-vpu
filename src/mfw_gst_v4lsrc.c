...
static GstBuffer * mfw_gst_v4lsrc_buffer_new(MFWGstV4LSrc *v4l_src,gint num);
...
DESCRIPTION:   This funtion registers the  buffer type 
               on to the V4L Source plugin

ARGUMENTS PASSED:
        None
  
RETURN VALUE:
        return the registered buffer type
...
DESCRIPTION:   This funtion initialises the buffer class of the V4l source
                plug-in
...
static gint mfw_gst_v4lsrc_start_capturing(MFWGstV4LSrc *v4l_src)
{
        ...
        /* query for TEST_BUFFER_NUM number of buffers 
        ...
        for (i = 0; i < TEST_BUFFER_NUM; i++)
        {
            ...
            {
                g_print("VIDIOC_QUERYBUF error\n");
                return -1;
            }
            v4l_src->buffers[i] = (GstBuffer *) gst_mini_object_new (MFW_GST_TYPE_V4LSRC_BUFFER);
            MFW_GST_V4LSRC_BUFFER (v4l_src->buffers[i])->v4lsrccontext = gst_object_ref (v4l_src);
...
static gint mfw_gst_v4lsrc_stop_capturing(MFWGstV4LSrc *v4l_src)
{
        ...
        {
            g_print("error in VIDIOC_STREAMOFF\n");
            return - 1;
        }
        for (i = 0; i < TEST_BUFFER_NUM; i++)
        {
            munmap(GST_BUFFER_DATA(v4l_src->buffers[i]),
                GST_BUFFER_SIZE(v4l_src->buffers[i]));
            gst_mini_object_unref((GstMiniObject *)v4l_src->buffers[i]);
...
static gint mfw_gst_v4lsrc_capture_setup(MFWGstV4LSrc *v4l_src)
{
        ...
        {
                g_print("v4l_mfw_gst_v4lsrc_capture_setup: VIDIOC_REQBUFS failed\n");
                return 0;
        }
...
mfw_gst_v4lsrc_overlay_setup(MFWGstV4LSrc *v4l_src, struct v4l2_format *fmt)
{
	 ...
	 ctl.id = V4L2_CID_PRIVATE_BASE+0;
	 ctl.value = g_rotate;
...
mfw_gst_v4lsrc_start (GstBaseSrc * src)
{
    ...
    if(TRUE==v4l_src->preview)
    {
        ...
        if (mfw_gst_v4lsrc_overlay_setup(v4l_src,&fmt) == FALSE) {
            g_print("Setup overlay failed.\n");
            return FALSE;
        }
        
        /* open the frame buffer to display the preview  */
        if ((fd_fb = open(fb_device, O_RDWR )) < 0)	{
            g_print("Unable to open frame buffer\n");
            return FALSE;
        }
        ...
        if (!g_overlay) {
            ...
            if (ioctl(v4l_src->fd_v4l, VIDIOC_G_FBUF, &fb_v4l2) < 0) {
                g_print("Get framebuffer failed\n");
                return FALSE;
            }
        ...
        {
            g_print("set framebuffer failed\n");
            return FALSE;
        } 
...
static GstBuffer * 
mfw_gst_v4lsrc_buffer_new(MFWGstV4LSrc *v4l_src,gint num)
{
    ...
    if (ioctl (v4l_src->fd_v4l, VIDIOC_DQBUF, &v4lbuf) < 0)	{
        g_print("VIDIOC_DQBUF failed.\n");
        return NULL;
    }
    buf = (v4l_src->buffers[v4lbuf.index]);
    GST_BUFFER_FLAG_SET (buf, GST_BUFFER_FLAG_LAST);

    if (v4l_src->count >= TEST_BUFFER_NUM) {
    ...
    GST_BUFFER_TIMESTAMP (buf) = gst_clock_get_time (GST_ELEMENT (v4l_src)->clock);

    GST_BUFFER_TIMESTAMP (buf) -= GST_ELEMENT (v4l_src)->base_time;
...
mfw_gst_v4lsrc_class_init(MFWGstV4LSrcClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, MFW_V4L_SRC_ROTATE,
				    g_param_spec_int("rotate",
                             ...
                             "can be specified. Rotation angles " 
