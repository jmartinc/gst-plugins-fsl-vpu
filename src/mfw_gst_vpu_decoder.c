...
static void vpu_mutex_lock(GMutex *mutex)
{
    return;
...
static void vpu_mutex_unlock(GMutex *mutex)
{
    return;
...
static gboolean mfw_gst_get_timestamp(MfwGstVPU_Dec *vpu_dec, GstClockTime * ptimestamp)
{
    ...
    if (found){
        if (index != vpu_dec->ts_tx){
            vpu_dec->timestamp_buffer[index] = vpu_dec->timestamp_buffer[vpu_dec->ts_tx];
        }
...
static gint mfw_gst_vpudec_FrameBufferInit(MfwGstVPU_Dec *vpu_dec,
                                           ...
                                           gint num_buffers)
{
    ...
    for(i=0;i<num_buffers;i++){
        retval = gst_pad_alloc_buffer_and_set_caps(vpu_dec->srcpad, 0,
            vpu_dec->outsize,GST_PAD_CAPS(vpu_dec->srcpad), &outbuffer);
...
IMPORTANT NOTES:    None
=======================================================================================*/
GstFlowReturn mfw_gst_vpudec_stream_buff_read_init(MfwGstVPU_Dec *vpu_dec, GstBuffer *buffer)
{
    RetCode vpu_ret=RETCODE_SUCCESS;
            ...
            for (i=0;i<vpu_dec->numframebufs;i++)
            {
                ...
                if (gst_buffer_is_writable(pBuffer))
                {
                    if (vpu_dec->fb_state_plugin[i] == FB_STATE_ALLOCTED)
                        numFreeBufs++;
                    else if (vpu_dec->fb_state_plugin[i] == FB_STATE_DISPLAY)
                    {
                        vpu_dec->fb_state_plugin[i] = FB_STATE_ALLOCTED;
...
GstFlowReturn mfw_gst_vpudec_vpu_init(MfwGstVPU_Dec *vpu_dec)
{
    ...
    if (vpu_dec->initialInfo->minFrameBufferCount>NUM_MAX_VPU_REQUIRED){
        g_print("vpu required frames number exceed max limitation, required %d.",vpu_dec->initialInfo->minFrameBufferCount);
        return(GST_FLOW_ERROR);
    }
    ...
    /* Padding the width and height to 16 */
    {
		...
		caps = gst_caps_new_simple("video/x-raw-yuv",
                    ...
                    "width", G_TYPE_INT,
                        ((vpu_dec->rotation_angle == 90) || (vpu_dec->rotation_angle==270)) ? vpu_dec->initialInfo->picHeight : vpu_dec->initialInfo->picWidth, 
                    "height", G_TYPE_INT,
                       ((vpu_dec->rotation_angle == 90) || (vpu_dec->rotation_angle==270)) ? vpu_dec->initialInfo->picWidth : vpu_dec->initialInfo->picHeight, 
                    ...
                    "crop-right-by-pixel", G_TYPE_INT,
                       ((vpu_dec->rotation_angle == 90) || (vpu_dec->rotation_angle==270)) ? (crop_right_len+7)/8*8 : (crop_bottom_len+7)/8*8,
                    "crop-bottom-by-pixel", G_TYPE_INT,
                       ((vpu_dec->rotation_angle == 90) || (vpu_dec->rotation_angle==270)) ? crop_right_len : crop_bottom_len,
    ...
    if(vpu_dec->framebufinit_done==FALSE)
    {
        if((mfw_gst_vpudec_FrameBufferInit(vpu_dec,vpu_dec->frameBuf,needFrameBufCount))< 0)
		{
            ...
            mfw_gst_vpudec_post_fatal_error_msg(vpu_dec,"Allocation of the Frame Buffers Failed");
    ...
    vpu_ret = vpu_DecRegisterFrameBuffer(*(vpu_dec->handle),
            vpu_dec->frameBuf,
            (vpu_dec->rotation_angle || vpu_dec->mirror_dir) ? vpu_dec->initialInfo->minFrameBufferCount : vpu_dec->numframebufs,
            vpu_dec->initialInfo->picWidth,
            &bufinfo);
    if (vpu_ret != RETCODE_SUCCESS) {
        GST_ERROR("vpu_DecRegisterFrameBuffer failed. Error code is %d \n",vpu_ret);
        mfw_gst_vpudec_post_fatal_error_msg(vpu_dec,"Registration of the Allocated Frame Buffers Failed ");
        return(GST_FLOW_ERROR);
    }
	...
	if (vpu_dec->rotation_angle || vpu_dec->mirror_dir)
	{
		...
		if (vpu_dec->rotation_angle)
		{
			...
			vpu_ret = vpu_DecGiveCommand(*(vpu_dec->handle),SET_ROTATION_ANGLE, &vpu_dec->rotation_angle);
		...
		{
            vpu_ret = vpu_DecGiveCommand(*(vpu_dec->handle),SET_MIRROR_DIRECTION, &vpu_dec->mirror_dir);
		}
        vpu_ret = vpu_DecGiveCommand(*(vpu_dec->handle),SET_ROTATOR_STRIDE, &rotStride);
	    if (vpu_ret != RETCODE_SUCCESS) {
            GST_ERROR("vpu_Dec SET_ROTATOR_STRIDE failed. ret=%d \n",vpu_ret);
            mfw_gst_vpudec_post_fatal_error_msg(vpu_dec,"VPU SET_ROTATOR_STRIDE failed ");
            return(GST_FLOW_ERROR);
		}
		vpu_dec->rot_buff_idx = vpu_dec->initialInfo->minFrameBufferCount;
        vpu_ret = vpu_DecGiveCommand(*(vpu_dec->handle),SET_ROTATOR_OUTPUT, &vpu_dec->frameBuf[vpu_dec->rot_buff_idx]);
		if (vpu_dec->rotation_angle)
...
IMPORTANT NOTES:    None
=======================================================================================*/
static GstFlowReturn mfw_gst_vpudec_chain_stream_mode(GstPad *pad, GstBuffer *buffer)
{
    ...
    if(G_UNLIKELY(!vpu_dec->vpu_opened)) {
        ...
        if (retval != GST_FLOW_OK)
        {
            GST_ERROR("mfw_gst_vpudec_stream_buff_read failed. Error code is %d \n",retval);
            goto done;
        }
    }
    ...
    if (retval != GST_FLOW_OK)
    {
        GST_ERROR("mfw_gst_vpudec_stream_buff_read_init failed. Error code is %d \n",retval);
        goto done;
    }
    ...
    while(1)
    {
        ...
        if (vpu_dec->is_startframe == FALSE)
        {
            ...
            if ((vpu_dec->loopback == FALSE) && (vpu_dec->rotation_angle == 0) && (vpu_dec->mirror_dir==MIRDIR_NONE))
            {
                vpu_mutex_lock(vpu_dec->vpu_mutex);
                retval = GST_FLOW_OK;
                goto done;
        ...
        if(G_LIKELY(vpu_dec->is_startframe == TRUE)){
...
#endif
            if ((vpu_dec->decParam->prescanEnable == 1) && (vpu_dec->outputInfo->prescanresult == 0))
            {
            ...
            if(G_LIKELY(vpu_dec->direct_render==TRUE))
            {
                if (vpu_dec->rotation_angle || vpu_dec->mirror_dir)
                {
                    ...
                    vpu_dec->rot_buff_idx = (vpu_dec->rot_buff_idx == vpu_dec->initialInfo->minFrameBufferCount) ? vpu_dec->initialInfo->minFrameBufferCount+1 : vpu_dec->initialInfo->minFrameBufferCount;
                ...
                retval = gst_pad_alloc_buffer_and_set_caps(vpu_dec->srcpad, 0,
                                  vpu_dec->outsize, GST_PAD_CAPS(vpu_dec->srcpad), &vpu_dec->pushbuff);
                if(retval != GST_FLOW_OK){
                    GST_ERROR("Error in allocating the Framebuffer[%d]," " error is %d",vpu_dec->prv_use_idx,retval);
                    goto done;
                }
            ...
            if (vpu_dec->codec == STD_MPEG2) 
			{
                ...
                if (!(mfw_gst_get_timestamp(vpu_dec, &ts))){
                    ...
                    if(vpu_dec->frame_rate != 0) 
                    {
                        ...
                        time_val = (gfloat) ((gfloat) vpu_dec->no_ts_frames / 25.0 );
                        ts = vpu_dec->base_ts + time_val * 1000 * 1000 * 1000;
                    }
                } else {
                    vpu_dec->base_ts = ts;
...
IMPORTANT NOTES:    None
=======================================================================================*/
static GstFlowReturn mfw_gst_vpudec_chain_file_mode(GstPad *pad, GstBuffer *buffer)
{
    MfwGstVPU_Dec *vpu_dec  = MFW_GST_VPU_DEC(GST_PAD_PARENT(pad));
    RetCode vpu_ret=RETCODE_SUCCESS;
    GstFlowReturn retval=GST_FLOW_OK;
    ...
    if(G_UNLIKELY(vpu_dec->init==FALSE)){
        ...
        if(vpu_dec->initialInfo->picWidth < MIN_WIDTH || vpu_dec->initialInfo->picHeight < MIN_HEIGHT) {
            ...
            gerror = g_error_new_literal(1, 0, text_msg);
            message =
                gst_message_new_error(GST_OBJECT(GST_ELEMENT(vpu_dec)), gerror,
                        "debug none");
            gst_element_post_message(GST_ELEMENT(vpu_dec), message);
            g_error_free(gerror);
...
#ifndef VPU_MX27
    if ((vpu_dec->outputInfo->interlacedFrame) && (vpu_dec->lastframedropped)){
        vpu_dec->ts_tx = (vpu_dec->ts_tx+1)%MAX_STREAM_BUF;
    }
#endif
...
static GstStateChangeReturn mfw_gst_vpudec_change_state(GstElement *element, GstStateChange transition)
{
   ...
   // gint i=0,err;
    ...
    switch (transition) 
    {
        case GST_STATE_CHANGE_NULL_TO_READY:
	    {
            ...
            if (vpu_ret) {
                GST_DEBUG("Error in geting the VPU version\n,error is %d",vpu_ret);
                vpu_UnInit();
                return GST_STATE_CHANGE_FAILURE;
            }
...
mfw_gst_vpudec_setcaps(GstPad * pad, GstCaps *caps)
{
   ...
   const gchar *mime;
    ...
    if(vpu_dec->codec == STD_MPEG2)
        gst_pad_set_chain_function(vpu_dec->sinkpad, mfw_gst_vpudec_chain_stream_mode);
...
GType mfw_gst_vpudec_mirror_get_type(void)
{
  ...
  if (!vpudec_mirror_type) {
    vpudec_mirror_type = g_enum_register_static ("MfwGstVpuDecMirror", vpudec_mirror);
  }
...
mfw_gst_vpudec_init(MfwGstVPU_Dec *vpu_dec, MfwGstVPU_DecClass *gclass)
{
    ...
    gst_pad_set_chain_function(vpu_dec->sinkpad, mfw_gst_vpudec_chain_stream_mode);
#else
    gst_pad_set_chain_function(vpu_dec->sinkpad, mfw_gst_vpudec_chain_file_mode);
#endif
